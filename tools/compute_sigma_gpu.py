...
def imgDistanceMatrix(img_fs):
    A = gpu.garray(img_fs)
    BT = gpu.garray(img_fs).transpose()
    vecProd = A * BT
    SqA =  gpu.square(A)
    sumSqA = gpu.sum(SqA, axis=1)
    sumSqAEx = gpu.tile(sumSqA.transpose(), (1, vecProd.shape[1]))    
    SqB =  gpu.square(A)
    sumSqB = gpu.sum(SqB, axis=1)
    sumSqBEx = gpu.tile(sumSqB, (vecProd.shape[0], 1))    
...
def sigma(imgDis):
    ...
    denominator = np.divide(gpu.multiply(length, length-1), 2)
    nominator = np.divide(gpu.sum(imgDis), 2)
    sigma = np.divide(nominator, denominator)
    return sigma

def KDE(img_fs, sigma):
    return gpu.exp(- np.divide(img_fs, gpu.square(sigma) ) )
...
if __name__ == '__main__':
    ...
    np.save('data/sigma_training.npy', sigma_training)
