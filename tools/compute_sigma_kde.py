...
def sigmaTensor(imgDis):
    ...
    sigma = tf.divide(nominator, denominator)
    return sigma
...
if __name__ == '__main__':
    with tf.Session() as sess:
        with tf.device("/gpu:0"):
            ...
            # img_fs_training = np.array([[1,2,3,4,5],[1,2,3,4,6],[1,2,3,4,6],[1,2,3,4,6],[1,2,3,4,7]], dtype = 'float64')
            print 'vgg_feats.shape:', img_fs.get_shape
            # if img_fs.dtype != 'float64':
            #     print 'converting %s to float64'%img_fs.dtype
            #     img_fs.dtype = 'float64'
            dis_all = imgDistanceTensor(img_fs)
            print 'dis_all.shape:', dis_all.get_shape()
            np.save('imgDistance.npy', dis_all.eval())
            ...
            if os.path.isfile(sigma_path):
                ...
                print 'KDE Matrix Shape:', kde_all.get_shape
                np.save('kde_matrix.npy', kde_all.eval())
